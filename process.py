...
if salary_file:
    ...
    for component, codes in calc_groups.items():
        ...
        if matched_cols:
            ...
            temp["Profit Centre"] = temp["Cost Centre"].map(profit_mapping).fillna(0).astype(int)
    ...
    if summary_list:
        ...
        # ---- Aggregate Deductions by Profit Centre ----
        ...
        rev_profit_map = {v: k for k, v in profit_mapping.items()}
        deductions_grouped["Cost Centre"] = deductions_grouped["Profit Centre"].map(rev_profit_map)
        deductions_grouped = deductions_grouped[["GL", "Cost Centre", "Profit Centre", "Component", "Amount"]]

        summary = pd.concat([earnings, deductions_grouped], ignore_index=True)

        # ---- Add Salary & Wages Payable ----
